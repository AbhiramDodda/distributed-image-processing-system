...
const (
	RaftTransport_Step_FullMethodName = "/raftgrpc.v1.RaftTransport/Step"
)
...
//
// RaftTransport carries Raft protocol messages between coordinator peers. It is a
// dumb pipe: the payload is a raftpb.Message marshaled by the consensus layer, so
// this service never needs to understand Raft semantics -- it only has to deliver
// bytes, and may drop them (Raft retransmits).
type RaftTransportClient interface {
	// Step delivers one marshaled raftpb.Message to a peer's Raft node.
	Step(ctx context.Context, in *RaftEnvelope, opts ...grpc.CallOption) (*RaftAck, error)
}
...
func (c *raftTransportClient) Step(ctx context.Context, in *RaftEnvelope, opts ...grpc.CallOption) (*RaftAck, error) {
...
type RaftTransportServer interface {
	// Step delivers one marshaled raftpb.Message to a peer's Raft node.
	Step(context.Context, *RaftEnvelope) (*RaftAck, error)
...
func (UnimplementedRaftTransportServer) Step(context.Context, *RaftEnvelope) (*RaftAck, error) {
...
func _RaftTransport_Step_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	...
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(RaftTransportServer).Step(ctx, req.(*RaftEnvelope))
	}
...
var RaftTransport_ServiceDesc = grpc.ServiceDesc{
	ServiceName: "raftgrpc.v1.RaftTransport",
	...
	Metadata: "internal/consensus/raftgrpc/raft.proto",
}
