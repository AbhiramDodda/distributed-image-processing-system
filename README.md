...
- Output formats for ML frameworks: committed task results are written columnar as **Parquet** — each worker stages a single-row-group result object the coordinator promotes into the `results/` tree, so completed jobs are queryable directly by Athena/Spark/DuckDB with no load step. The `internal/formats` package additionally provides TFRecord (masked CRC32C framing, interoperable with `tf.data`), WebDataset tar shards (grouped by sample key, deterministic ordering), and Apache Arrow columnar batches with IPC transport — correctly-framed, tested encoders ready to wire, though only Parquet result output is currently on a runtime path.
